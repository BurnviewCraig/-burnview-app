...
export default function TimeBookPage() {
  ...
  const [section, setSection] = useState<"DAIRY" | "STAFF">("DAIRY");
  ...
  const switchSection = (s: "DAIRY" | "STAFF") => {
    setSection(s);
    setAdding(false);
  };
  ...
  const handleAddWorker = async () => {
    ...
    await fetch("/api/workers", {
      ...
      body: JSON.stringify({ farmId: farm.id, section, name: newName.trim() }),
    });
    setNewName("");
    setAdding(false);
    refetchWorkers();
  ...
  return (
    <div className="screen">
      ...
      <div className="grazing-week-nav no-print">
        ...
        <div style={{ flex: 1 }} />
        <button className="link-btn" onClick={() => setAdding((v) => !v)}><Plus size={14} style={{ verticalAlign: "-2px" }} /> Add worker</button>
        <button className="link-btn" onClick={() => window.print()}><Printer size={14} style={{ verticalAlign: "-2px" }} /> Print</button>
      </div>
      ...
      <p className="timebook-title-print">{farm.name} — {TIMEBOOK_SECTIONS.find((s) => s.id === section)?.name} — week of {anchor}</p>

      {workers.length === 0 ? (
        <p className="ds-note" style={{ padding: "12px 18px" }}>No workers added for this section yet — add one above.</p>
      ) : (
        <div className="grazing-table-scroll">
          <table className="grazing-table timebook-table">
            ...
            <tbody>
              {workers.map((w) => (
                <tr key={w.id}>
                  <td className="grazing-table-label timebook-name" onClick={() => setManagingWorker(w)}>{w.name}</td>
                  {days.map((d) => {
